...
4. **预览**：`.claude/launch.json` 已配 `deck`（`python3 -m http.server 4507`）。启动后开 `http://localhost:4507/index.html` —— 带键盘翻页（←/→）和全屏放映（F）。
5. **导出**（会自动先跑 `renumber.py` 刷页码）：
...
├─ scripts/
│   ├─ reskin.py           一键换肤：主色+公司名 → 自动生成 theme.css（支持 --dark/--rename）
